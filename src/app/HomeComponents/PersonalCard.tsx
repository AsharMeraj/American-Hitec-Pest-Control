...
const PersonalCard = ({ name, para, img, position }: PropType) => {
    return (
        <div className='flex flex-col md:flex-row items-center md:shadow-lg md:shadow-black/5'>
            <div className='w-fit md:w-auto xl:pt-[1rem] bg-black/5'>
            ...
            <div className={`w-full flex  items-center `}>
                <div className={`max-w-[45rem] md:ml-8 mt-4 md:mt-0`}>
                    <h2 className={`text-xl md:2xl xl:text-3xl font-extrabold uppercase text-center md:text-start`}>{name}</h2>
                    <h2 className={`text-xl md:text-2xl font-bold text-[--Primary] md:mt-2 text-center md:text-start`}>{position}</h2>
